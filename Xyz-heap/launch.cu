...
cudaError_t standard::launch(Params param, cudaStream_t stream) {
  cudaError_t result;
  ...
  if (grid_k == 1) {
    ...
    result = cudaMalloc(&param.work, grid_k * param.size_p.kRow *
                                         param.size_p.kColumn * sizeof(Type));
  ...
  compute<<<grid, block, 0, stream>>>(param);
  ...
  reduce<<<grid, block, 0, stream>>>(param);
