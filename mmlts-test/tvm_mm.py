    ...
    def main(A: T.Buffer[(N, L), "float32"],
        ...
        for i, j, k in T.grid(N, L, M):
            with T.block("C"):
                ...
                with T.init():
                    C[vi, vj] = 0.0
                C[vi, vj] = C[vi, vj] + A[vi, vk] * B[vk, vj] + (A[vi, vk] * B[vk, vj] > R[0, vj]).astype("float32") * (A[vi, vk] * B[vk, vj] - R[0, vj])
...
r_np = np.random.uniform(size=(1, M)).astype(np.float32)

dev = tvm.cuda()
a_tvm = tvm.nd.array(a_np, device=dev)
b_tvm = tvm.nd.array(b_np, device=dev)
r_tvm = tvm.nd.array(r_np, device=dev)
c_tvm = tvm.nd.empty((N, M), device=dev)

func(a_tvm, b_tvm, r_tvm, c_tvm)

evaluator = func.time_evaluator(func.entry_name, dev, number=10)
print(
    "Execution time of this operator: %.3f ms"
    % (np.median(evaluator(a_tvm, b_tvm, r_tvm, c_tvm).results) * 1000)
)
